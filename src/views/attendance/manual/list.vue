    ...
    <div>
        ...
        <List :listUrl="listUrl" :columns="tableColumns" ref="list" :query="listQuery" :data="listData">
            <ListHeader slot="header" mode="single">
                ...
                <ListOperations>
                    <Button type="primary" icon="plus-round" size="small" @click="$router.push('/attendance/manual/create')">签卡登记</Button>
                </ListOperations>
            </ListHeader>
        </List>
    </div>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            update (id) {
                // 模板字符串
                this.$router.push(`/attendance/manual/edit/${id}`)
            },
            del () {
                if (this.currentDeleteId === null) return
                del('attendanceRecord', this.currentDeleteId).then(response => {
                    this.delModal = false
